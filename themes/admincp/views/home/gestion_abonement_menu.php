<?php $userInfo = WpUser::model()->findByAttributes(array('ID'=>$model->user_id)); ?>
<ul class="immobilier-ul" style="margin-top:10px;">
    <?php
    $abonnement_premium_bon_commande = Yii::app()->db->createCommand()->select('*')->from('abonnement_premium_bon_commande')
        ->where('valide=1 AND post_property_id = '.$model->id)->queryRow();
    if ($abonnement_premium_bon_commande !== false && $model->type_property == 1 && $model->getCoutCommande() > 0): ?>
        <li><a class="acquereursForm-modal" data-fancybox-type="ajax" href="<?php echo PIUrl::createUrl('/gestionAbonnement/validationCheque', array('id' => $model->id)); ?>"><span class="color-icons arrow_right_co"></span> Validation du ch&egrave;que en attente</a></li>
    <?php endif; ?>
    ...
    <?php if ($model->type_property == 1 && $model->payment_id == 7 && ($model->status == 0 && $model->is_validate == 1)): ?>
        ...
        <li>
            <a rel="gallery" class="Creationcontratprivilegemail" data-fancybox-type="iframe" href="<?php echo Yii::app()->createAbsoluteUrl('creationcontratprivilegemail/creationcprivileevmail/'.$model->id);?>">
                <span class="color-icons arrow_right_co"></span>Création contrat privilège & envoi par mail</a>
        </li>
        <li><a href="<?php echo Yii::app()->createAbsoluteUrl('gestionAbonnement/envoiMailInfomation');?>?id_m=<?php echo $model->id;?>&action=maPremium"><span class="color-icons arrow_right_co"></span> Envoi mail information concernant les formules</a></li>
    <?php elseif ($model->type_property == 1 && $model->payment_id == 2): #ANNONCE EN STATU PRIVILEGE ?>
        ...
        <li>
            <a  rel="gallery" id="<?php echo $model->id; ?>" class="envoimailcontentieux" href="javascript:void(0);"><span class="color-icons arrow_right_co"></span>Envoi mail contentieux</a>
        </li>
        <?php if ($model->status == 0 && $model->is_validate == 1): #EN LIGNE  ?>
            <?php if (dateDiff($model->date_mail, date('Y-m-d H:i:s')) >= 1)://if (true):#dateDiff() > 1):?>
...
//                    exit; 
            ...
            <?php endif; ?>
            <li>
                <a href="<?php echo Yii::app()->request->hostInfo . '/accesmembres/?loginUser='.hashCrypte($model->user_id).'&actionTo=payment&hash='.sha1($userInfo->user_email).'&property_id='.$model->id; ?>">
                    <span class="color-icons arrow_right_co"></span> Souscription d'un nouvel abonnement</a>
            </li>

            <li> <a rel="gallery" class="Creationcontratprivilegemail" data-fancybox-type="iframe" href="<?php echo Yii::app()->createAbsoluteUrl('creationcontratprivilegemail/creationcprivileevmail/'.$model->id);?>">
    ...
    <?php elseif ($model->type_property == 1 && ($model->payment_id == 3 || $model->payment_id == 4) && ($model->status == 0 AND $model->is_validate == 1 AND $model->status != 83)): #DESACTIVATION ANNONCE DE VENTE PREMIUM ?>
        <?php if ($model->payment_id == 3 && $model->getExistsPaypalProfile() > 0): #VERIF SI ABONNEMENT PAYPAL EN COURS ?>
            <!-- <li><a rel="gallery" href="javascript:void(0);" ref="<?php echo $model->id;?>" class="premiumPaypal"><span class="color-icons arrow_right_co"></span> <strong>R&eacute;silier l'abonnement Premium (Paypal)</strong></a></li> -->
            <li><a class="PaypalResilier" data-fancybox-type="iframe" href="<?php echo PIUrl::createUrl('/module/paypalResilier?id=' . $model->id); ?>"><span class="color-icons arrow_right_co"></span><strong>R&eacute;silier l'abonnement Premium (Paypal)</strong></a></li>
        ...
        <?php else: ?>
            <li><a class="PremiumResilier" data-fancybox-type="iframe" href="<?php echo PIUrl::createUrl('/module/premiumResilier?id=' . $model->id); ?>"><span class="color-icons arrow_right_co"></span>R&eacute;silier l'abonnement Premium</a></li>
        <?php endif; ?>
    <?php elseif ($model->type_property == 1 && ($model->status != 0 || $model->is_validate != 1)): #ACTIVATION ANNONCE DE VENTE ?>
        ...
        <li>
            <a rel="gallery" class="Creationcontratprivilegemail" data-fancybox-type="iframe" href="<?php echo Yii::app()->createAbsoluteUrl('creationcontratprivilegemail/creationcprivileevmail/'.$model->id);?>" id="<?php echo $model->id; ?>">
                <span class="color-icons arrow_right_co"></span>Création contrat privilège & envoi par mail</a>
        </li>

    <?php elseif ($model->type_property == 2): #LOCATION  ?>
        <?php
        //if ($model->status == 1 || $model->is_validate == 1)
        if ($model->status == 0 || $model->is_validate == 0):
            ?>
            <a href="javascript:void(0);" id="wd-activeProperty1" rel="<?php echo $model->id; ?>"><span class="color-icons arrow_right_co"></span><?php echo $model->is_validate == 0 ? "Activer l'annonce" : "Désactiver l'annonce"; ?></a>
    ...
    $(document).ready(function() {
        $("#Desactiverlannonce").click(function() {
            ...
            $.ajax({
                ...
                data: {'id': id},
                success: function(data) {
                    alertify.alert(" Désactiver l'annonce succès.",function(){
    ...
    $(document).ready(function() {
        $("#Reactiverlannonce").click(function() {
            ...
            $.ajax({
                ...
                data: {'id': id},
                success: function(data) {
                    alertify.alert(" Reactiver l'annonce succès.",function(){
                        window.location.reload();
                    });
                }
            });
...
//    $(document).ready(function() {
//        $(".envoimailcontentieux").click(function() {
//            var id = $(this).attr('rel');
//            var usermail =  $("#usermail").val();
//
//            $.ajax({
//                type: 'POST',
//                url:webroot + '/envoimailcontentieux/envoimail/',
//                data: {'id': id,'usermail':usermail},
//                success: function(data) {
//                    alertify.alert("OK");
//                }
//            });
