...
window.addEvent('domready', function(){
	...
	c = new Fx.Slide('commentaire');
	i = new Fx.Slide('information');
	d = $('raison');
...
function choixchanged(cb){
	
	switch( cb.getValue() ){
		case "Je ne suis pas satisfait de vos services": 
			i.slideIn();
			c.slideIn();
			break;
		case "Autre": 
...
function verifier(f){
	...
	var modecoche 	= false;
	var choixcoche 	= false;
	var choix = "";
		
	$each($$('input[name="mode"]'),function(checkbox, i) {
		if ( checkbox.getProperty('checked') ){
			modecoche = true;			
		}
	});
			
	$each($$('input[name="raison"]'),function(checkbox, i) {
		if ( checkbox.getProperty('checked') ){
			choixcoche = true;
			choix = checkbox.getValue();
		}
	});
	...
	/* Si autre demande minimum de commentaire
	if( choix=="Autre" && $('COMM_CLIENT').getValue().length<10 ){
		alert("Veuillez saisir un minimum de 10 caractères dans la cellule commentaire.");
		$('COMM_CLIENT').focus();
		return false;
	}
	*/
...
<!-- CORPS DE LA PAGE -->
<table border="0" cellspacing="0" cellpadding="0" width="730px" class="compte">
<tr>
	<td>
		<!-- MENU -->
				...
				if ($data['nb_visualisation'] >= $nbminvisualisation) { ?>
                	...
                	<span style="font-size:14px;">
						...
						if ($data['nb_photo'] < 1) { ?>
                        	...
                        	<img src="/images/puces/v<?php= nbrem ?>.gif" border="0" align="absmiddle" />
