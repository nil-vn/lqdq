...
<form method="post" action="" name="form" id="form_resilier">
<input name="post" type="hidden" id="action_post" value="1" />
<input name="id" type="hidden" id="action_id" value="<?php echo $data['id'];?>" />
<input name="hash" type="hidden" id="action_hash" value="<?php echo $data['hash'];?>" />         
<tr>
    <td>        
            ...
            <div style="padding-left:200px; font-size:14px;">
                ...
                <label>
                <input type="radio" name="raison" class="raison" value="J'ai vendu grâce a Immobilier.fr">J'ai vendu gr&acirc;ce &agrave; Immobilier.fr</option>
                </label>
                <br/>
                <label>
                <input type="radio" name="raison" class="raison" value="J'ai vendu par un autre biais">J'ai vendu par un autre biais</option>
                </label>
                <br/>
                <label>
                <input type="radio" name="raison" class="raison" value="Je ne souhaite plus vendre">Je ne souhaite plus vendre</option>
                </label>
                <br/>
                <label>
                <input type="radio" name="raison" class="raison" value="Je reporte la mise en vente pour plus tard">Je reporte la mise en vente pour plus tard</option>
                </label>
                <br/>
                <label>
                <input type="radio" name="raison" class="raison" value="Je ne suis pas satisfait de vos services">Je ne suis pas satisfait de vos services</option>
                </label>
                <br/>
                <label>
                <input type="radio" name="raison" class="raison" value="Autre">Autre raison</option>
                </label>
                <br/>
                ...
                if ($data['nb_visualisation'] >= $nbminvisualisation) { ?>
                    ...
                    <span style="font-size:14px;">
                        ...
                        if ($data['nb_photo'] < 1) { ?>
                            ...
                            <br/>
                            <center>
                            <button onclick="location.replace('../photos/photos.php?uid=<?php echo 'uid';//annonce.contact.uid ?>&id=<?php echo $data['id'];?>');">
                        ...
                        <?php 
                            ...
                            <br/>
                            <center>
                            <button onclick="location.replace('../photos/photos.php?uid=<?php echo 'uid';//moncompte.annonce.contact.uid ?>&id=<?php echo $data['id'];?>');" >
                                <img src="/images/icones/png/photos.png" alt="" width="16" height="16" border="0" align="absmiddle" /> Ajouter des photos
                            </button>
                            </center>
                        <?php 
                        ...
                        if ($nbmots <= $nbminmots) { ?>
                            ...
                            <br/>
                            <center>
                            <button onclick="location.replace('../annonces/modifier.php?id=<?php echo $data['id'];?>&hash=<?php echo $data['hash'];?>');" >
            ...
            <div align="center" style="width:100%; background-color:#EEE; border-top:2px #FFF dotted; color:#333333">
                <br/>
                <input type="button" id="submitForm" name="Submit" value="Résilier mon abonnement >>" />
                <br/>
                <br/>
            </div>
        <br/>
...
$(document).ready(function(){
    $('#submitForm').live('click', function() {
        ...
        var post = $("#action_post").val();
        var raison = $(".raison:checked").val();
        var commentaire = $("#COMM_CLIENT").val();
        var data = {
            'id': id,
            'post': post,
            'raison': raison,
        ...
        $.ajax({
            url: webroot + '/module/paypalResilier',
            type: 'POST',
            dataType: 'json',
            data: data,
