<?php 
	...
	if (isset($_GET['ref'])) {
		$id_annonce_s = $model['id_annonce_s'];
		$id_annonce_b = $model['id_annonce_b'];
		$model['id_annonce_s'] = $_GET['ref'];
		$model['id_annonce_b'] = $_GET['ref'];
		if (!$model->save()) {
			?>
			<script>alert('Cannot save with this value!!!');</script>
			<?php
			$model['id_annonce_s'] = $id_annonce_s;
			$model['id_annonce_b'] = $id_annonce_b;
		}
	}
// server
