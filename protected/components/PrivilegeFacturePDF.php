 ...
 * @author huytbt
 */
class PrivilegeFacturePDF extends TCPDF
{
	...
	public function exportPDF($attrs=array())
	{
		...
		// 'Commission négociée
			...
			$this->Text ($this->GetX()-23, $this->GetY()+1, "Forfaitaire");
			$this->Text ($this->GetX()-30, $this->GetY()+5, "(suite négociations)");
		}
		$this->SetX($this->GetX()+2);
		$this->Cell (32,$aspTailleLigne, number_format(($prix_vente*($commission/100)),2), 0,1,"C", 0);
		...
		if (isset($attrs['return'])) {
			try{
				$this->Output(Yii::getPathOfAlias('webroot')."/compta/facture_privilege_".$id.".pdf","F");
				return array(
