 ...
 * @author thanhnt
 */
class CourrierSimplePDF extends TCPDF
{
	...
	public function exportPDF($attrs=array())
	{
		...
		$subject = "Courrier simple BTK".$property_id." (Mandat ".$id_mendat.")";
			
		$this->SetTitle("Immobilier.fr - Lettre simple mandat ".$id_mendat);
		...
		$this->Ln($aspTailleLigne*2);
		$this->SetX(25);
		$this->Text($this->GetX(), $this->GetY(), "Malgré plusieurs demandes par e-mail, vous ne nous avez pas fourni ces informations.");
		
		$this->Ln($aspTailleLigne);
		$this->SetX(25);
		$this->Text($this->GetX(), $this->GetY(), "Nos différents mails ne vous sont sans doute pas parvenus.");
		...
		if (isset($attrs['return'])) {
			try{
				$this->Output(Yii::getPathOfAlias('webroot')."/voidcourrier/courrier_simple_BTK".$property_id.".pdf","F");
				return array(
					'path'		=> Yii::getPathOfAlias('webroot')."/voidcourrier/",
					'filename'	=> "courrier_simple_BTK".$property_id."_v1.pdf",
				);
			}catch(Exception $e){
				return false;
			}
